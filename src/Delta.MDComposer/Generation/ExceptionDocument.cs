...
internal sealed class ExceptionDocument(Exception exception) : IDocument
{
    private const string exceptionIcon =
        """
    ...
    public void Compose(IDocumentContainer document) => document.Page(page =>
    {
        ...
        _ = page.Foreground().Border(12).BorderColor(Colors.Red.Medium);
        ...
        page.Header()
            ...
            .Row(row =>
            {
                ...
                _ = row.AutoItem().PaddingTop(15).Width(iconSize).Height(iconSize).Svg(exceptionIcon);

                row.RelativeItem().Column(column =>
                {
                    _ = column.Item().Text("Exception").FontSize(36).FontColor(Colors.Red.Medium).Bold();
                    _ = column.Item().PaddingTop(-5).Text("Please refer to the details for further insight and troubleshooting.").FontSize(18).FontColor(Colors.Red.Medium).Bold();
                });
            });
            ...
            while (currentException != null)
            {
                column.Item()
                    ...
                    .Text(text =>
                    {
                        ...
                        _ = text.Span(currentException.GetType().Name + ": ").Bold();
                        _ = text.Span(currentException.Message);
                    });

                var traces = currentException.StackTrace ?? "";
                foreach (var trace in traces.Replace("\r\n", "\n").Split('\n')) _ = column
                    .Item()
                    .ShowEntire()
                    .BorderBottom(1)
